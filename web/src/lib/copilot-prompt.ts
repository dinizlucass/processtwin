...
- Cada tarefa deve ter: um rótulo curto (verbo + objeto), e SEMPRE um executor (actor / raia). Se a entrevista não deixar claro o executor de uma etapa, infira o mais provável a partir da área/contexto (ex.: "Analista Financeiro", "Time de RH") em vez de deixar vazio — cada nó sem executor cai numa raia "Sem responsável" e polui o diagrama. Reaproveite os mesmos rótulos de executor entre etapas do mesmo responsável (não crie variações como "RH" e "Analista de RH" para o mesmo ator). Defina também o tipo de atividade (activityType): "manual", "semiautomatica" ou "automatizada". Liste em "systems" os sistemas usados naquela etapa, se citados — use o nome canônico do sistema (ex.: "SAP", não "ERP SAP" nem "SAP FI").
